...
- 구현한 내용 (위 그림 참조)
    - 상세한 UI 레이아웃 설계: Label, Button, ComboBox, TrackBar 등 다양한 WinForms 컨트롤을 배치하여 실제 그림판과 유사한 인터페이스 환경 구축
    - 논리적 그룹화: GroupBox 컨트롤을 사용하여 '도형 선택', '색 선택', '선 두께' 등 서로 연관된 기능들을 시각적으로 묶어 사용자 편의성 증대
    - 시각적 버튼 디자인: 각 도형 선택 버튼에 Image 속성과 ImageAlign(TopCenter)을 적용하여 아이콘과 텍스트가 조화롭게 표시되도록 설정
    - 캔버스 리소스 초기화: 프로그램 실행 시 PictureBox의 크기에 맞게 Bitmap 객체를 생성하고 Graphics.Clear(Color.White) 명령으로 흰색 배경의 도화지 상태 구현
    - 컨트롤 속성 커스터마이징: 콤보박스의 항목(Items) 구성 및 트랙바의 최소/최대 범위를 선 굵기(1~10)에 적절하게 설정하여 즉각적인 조작이 가능하도록 초기값 세팅
    - 일관된 명명 규칙 적용: 각 컨트롤에 btnLine, cmbColor, trbLineWidth 등 직관적인 이름을 부여하여 이벤트 핸들러 연결 및 코드 관리 효율성 강화
